  ...
  const handleImportAndAnalyze = async () => {
    ...
    try {
      const filesToProcess = files
        .filter(f => selectedFiles.includes(f.id) && f.file)
        .map(f => f.file as File);

      if (filesToProcess.length === 0) {
        throw new Error('No valid files selected for processing.');
      }

      // Update status to processing
      updateFilesStatus(selectedFiles, 'processing', 10);
      ...
      updateFilesStatus(selectedFiles, 'completed', 100);
      ...
      updateFilesStatus(selectedFiles, 'error', 0, error.message);
      setCurrentStep(3);
    }
  };
