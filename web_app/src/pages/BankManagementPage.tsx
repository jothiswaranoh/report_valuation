        ...
        <div className="min-h-screen bg-transparent p-8">
            <div className="container mx-auto max-w-6xl">
                ...
                <div className="flex justify-between items-end mb-10 fade-in">
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">Bank Management</h1>
                        <p className="text-lg text-slate-500 font-medium">Manage the list of banks available for reports</p>
                    </div>
                    <Button
                ...
                {/* Main Card */}
                <div className="bg-white rounded-xl shadow-xl border border-sky-100 overflow-hidden fade-in">
                    {/* Search Bar */}
                    <div className="p-6 border-b border-sky-100 bg-gradient-to-r from-white to-sky-50/30">
                        <div className="relative max-w-md">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-sky-400" size={20} />
                            <input
                                type="text"
                                placeholder="Search banks..."
                                className="w-full pl-12 pr-4 py-3 border border-sky-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-sky-400 transition-all shadow-sm"
                                value={searchQuery}
                    ...
                    <div className="overflow-x-auto custom-scrollbar">
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-sky-50/60 text-slate-600 font-bold text-sm border-b border-sky-100">
                            ...
                            <tbody className="divide-y divide-sky-50">
                                {filteredBanks.length === 0 ? (
                                    <tr>
                                        <td colSpan={2} className="px-8 py-12 text-center">
                                            <div className="flex flex-col items-center justify-center">
                                                <div className="w-16 h-16 bg-sky-100 rounded-lg flex items-center justify-center mb-4">
                                                    <Building2 className="h-8 w-8 text-sky-400" />
                                                </div>
                                                <p className="text-slate-600 font-medium">No banks found</p>
                                                <p className="text-sm text-slate-500 mt-1">Add a bank to get started</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    filteredBanks.map((bank, index) => (
                                        <tr
                                            key={bank.id}
                                            className="hover:bg-sky-50/40 transition-all duration-300 group cursor-default border-b border-sky-50 last:border-0"
                                            style={{ animationDelay: `${index * 30}ms` }}
                                        >
                                            <td className="px-8 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-sky-400 to-sky-600 flex items-center justify-center text-white shadow-md group-hover:scale-110 transition-transform duration-300">
                                                        <Building2 size={24} />
                                                    </div>
                                                    <span className="text-lg font-bold text-slate-800 group-hover:text-sky-600 transition-colors">{bank.name}</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button
                                                        ...
                                                        className="p-2.5 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-all duration-300 hover:shadow-sm"
                                                        title="Edit"
                                                    >
                                                        <Pencil size={20} />
                                                    </button>
                                                    <button
                                                        ...
                                                        className="p-2.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all duration-300 hover:shadow-sm"
                                                        title="Delete"
                                                    >
                                                        <Trash2 size={20} />
                                                    </button>
                ...
                >
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 border border-sky-200 rounded-lg focus:ring-2 focus:ring-sky-400/30 focus:border-sky-400 outline-none transition-all bg-sky-50/30"
                        />
                    </div>
                </Modal>
